...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    // CORRECCIÓN: destructurar correctamente el 'data' devuelto y proteger el unsubscribe
    const { data } = supabase.auth.onAuthStateChange((_event, session) => {
      setUser(session?.user ?? null);
      if (session?.user?.id) {
        // No await aquí por ser callback; se pueden ejecutar en segundo plano
  ...
  const signUp = async (email: string, password: string, fullName: string) => {
    try {
      ...
      // Si quieres garantizar que el perfil exista, usar upsert en lugar de update
